...
export function DocumentView({
  documentName, sourceQuote, sourceStart, sourceEnd, onClose,
}: DocumentViewProps) {
  ...
  const hasHighlight = !!(sourceQuote || (sourceStart != null && sourceEnd != null));
  ...
  return (
    <div className="h-full flex flex-col bg-slate-100 animate-in slide-in-from-right duration-300">
      <Header name={documentName} ext={sourceExt} url={sourceUrl}
        zoom={zoom} setZoom={setZoom} showZoom={viewMode !== 'source'}
      ...
      <div ref={scrollRef} className="flex-1 overflow-auto p-4">
        {loading ? (
          ...
          <SourceView docText={docText} sourceQuote={sourceQuote}
            sourceStart={sourceStart} sourceEnd={sourceEnd} zoom={zoom} />
        ) : viewMode === 'original' && sourceExt === 'pdf' && sourceUrl ? (
          <div style={{ height: 'calc(100vh - 160px)' }}>
            <PdfViewer url={sourceUrl} sourceQuote={sourceQuote} />
          </div>
        ) : viewMode === 'original' && sourceExt === 'docx' && sourceUrl ? (
          <DocxViewer url={sourceUrl} sourceQuote={sourceQuote}
            plainText={docText} sourceStart={sourceStart} sourceEnd={sourceEnd} />
        ) : docText ? (
          <SourceView docText={docText} sourceQuote={sourceQuote}
            sourceStart={sourceStart} sourceEnd={sourceEnd} zoom={zoom} />
        ) : (
...
function Header({ name, ext, url, zoom, setZoom, showZoom, viewMode, hasOriginal,
  hasHighlight, onToggleView, onClose }: {
  name: string; ext: string | null; url: string | null;
  zoom: number; setZoom: (fn: (z: number) => number) => void; showZoom: boolean;
